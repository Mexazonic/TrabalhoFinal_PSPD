...
int intervalos[1000][3];


// void write_image_part(const char* filename, int start, int end) {
//     FILE* file = fopen(filename, "w");
//     // Escreve uma parte da imagem no arquivo
//     for (int i = start; i <= end; i++) {
//         for (int j = 0; j < IMG_WIDTH; j++) {
//             fprintf(file, "%d %d %d ", i, j, (i + j) % 255);
//         }
//         fprintf(file, "\n");
//     }
//     fclose(file);
...
int main(int argc, char *argv[]) {
    ...
    FILE *output_file;
    // MPI_Bcast(&global_var, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  //   for (int i = intervalos[1][0]; i < intervalos[1][1] ; i++)
  //       for (int j = 0; j < largura*3; j+=3){
  //           compute_julia_pixel(j/3, i, largura, altura, 1.0, rgb);
  //           pixel_array[local_i]= rgb[0]; local_i++;
  //           pixel_array[local_i]= rgb[1]; local_i++;
  ...
  //   free(rgb);
  //   //escreve o cabeçalho do arquivo
  //   output_file= fopen("out_julia_normal.bmp", "w");
  //   write_bmp_header(output_file, largura, 5);
  //   //escreve o array no arquivo
  //   fwrite(pixel_array, sizeof(unsigned char), area, output_file);
  //   fclose(output_file);
  //   free(pixel_array);
